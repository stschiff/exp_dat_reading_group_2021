...
```{r,results=F}
packages <- c("tidyverse", "missMethods", "cowplot", "ggrepel", "shiny")
...
Let's play with these functions. Do they differ? Modify the `destruction_level` parameter! You can also do so with this tiny shiny app:
...
### Variable levels of missingness
A short brief of non-uniform missigness. This will randomly add between 10 and 90% missingness to data:

```{r}

p_basic = geno_matrix %>% prcomp %>% tidy_pca_output %>% plot_tidy_pca_simple
p_basic
Y = geno_matrix %>%
   shoot_holes_column_wise_nonunif(0.1, .9) 
...
Y %>%
  ...
  tidy_pca_output() %>%
  dplyr::mutate(
    missingness = apply(Y, 1, function(x) { sum(is.na(x)) })/ncol(Y)
  ) %>%
  ggplot() +
  geom_point(
    aes(x = PC1, y = PC2, colour = missingness ),
    size = 3
  ) +
  scale_color_viridis_c()

#compare to softimpute
require(softImpute)
y_softimpute = Y %>% scale(scale=F) %>% softImpute
y_softimpute$x = y_softimpute$u
colnames(y_softimpute$x) = sprintf("PC%d", 1:ncol(y_softimpute$x))
p_softimpute = y_softimpute %>% tidy_pca_output %>% plot_tidy_pca_simple()
p_softimpute
```


### Messy missingness
The setup here is that we want missingness to vary by macro-region
```{r}
library(dplyr)
df_missingness = context_info %>% select(Makro_Region) %>% distinct
df_missingness$missing = c(.2, .7, .3, .5, .2, .8, .6)
context_missing = context_info %>% left_join(df_missingness) %>% mutate(missing=runif(54,missing-.1, missing+.1))
head(context_missing)

shoot_holes_by_sample <- function(x, n_missing){
  n_samples = nrow(x)
  n_snps = ncol(x)
  n_missing = round(n_snps * n_missing)
...
```{r}
Y = geno_matrix %>%
   shoot_holes_by_sample(context_missing$missing) 

y_meanimpute = Y %>%
   patch_holes_mean %>%
   prcomp(geno_matrix) %>%
   tidy_pca_output(context_missing)
p_meanimpute = y_meanimpute %>% 
   plot_tidy_pca_simple()
p_meanimpute

p_meanimpute2 =   y_meanimpute %>% 
  ggplot() +
  geom_text(
    aes(x = PC1, y = PC2, colour = missing, label=substr(Makro_Region, 1,3 )),
    size = 5
  ) +
  scale_color_viridis_c()
p_meanimpute2
...
p_softimpute2 =   y_softimpute %>% tidy_pca_output(context=context_missing)%>% 
  ggplot() +
  geom_text(
    aes(x = PC1, y = PC2, colour = missing, label=substr(Makro_Region, 1,3 )),
    size = 5
