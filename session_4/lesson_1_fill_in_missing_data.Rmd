...
The R package `missingMethods` provides a number of imputation algorithms, including the following three for mean imputation: `impute_mean()`, `impute_median()`, and `impute_mode`. The first one should behave identically to our own implementation `patch_holes_mean()`. We can compare their effects with the helper function `explore_filling_method()`, which wraps the whole pipeline from the imputation to the plot.
...
[Play with these functions and modify the `destruction_level` parameter]

### Other imputation algorithms

There are many different ("multiple") imputation algorithms. Here's just a selection of algorithms for imputation of numerical data that could be applied to genotype data (to my understanding). I tried running some of them, but the either the runtime was too long for this large dataset, or the run failed for one reason or the other (e.g. memory overflows).
...
- `mice::mice(method = 'pmm')` (Predictive Mean Matching)
...
```{r}
geno_matrix %>% explore_filling_method(missMethods::impute_sRHD, destruction_level = 0.5)
#geno_matrix %>% explore_filling_method(missForest::missForest, destruction_level = 0.1)
```

More research is urgently necessary.

#### Imputation in classes

All of these algorithms can be applied to meaningful subsets of data to leverage known context information. The missMethods package provides a useful helper function `impute_in_classes` for that.

```{r}
geno_matrix[,1:5000] %>% # reduced dataset for performance reasons
  prcomp(geno_matrix) %>%
  tidy_pca_output() %>%
  plot_tidy_pca_simple()
...
[Compare the output of these grouped mean imputation to the simple mean imputation attempted further up]
